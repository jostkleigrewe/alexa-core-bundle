...
class AlexaIntentDispatcher
{
    ...
    private const string NO_INTENT_ERROR = 'No intent handler available for this request.';
    ...
    public function dispatch(AlexaRequest $alexaRequest): AlexaResponse
    {
        ...
        $intentName = $alexaRequest->getRequest()->getIntent()?->getName();
        ...
        try {
            ...
            if ($handler === null) {
                ...
                throw new AlexaDispatchException(self::NO_INTENT_ERROR . ' [' . ($intentName ?? $type) . ']');
            }
            ...
            $this->logger->critical('Unexpected error during dispatch', ['exception' => $e]);
            return $this->responseService->createErrorResponse($e);
        }
    }
