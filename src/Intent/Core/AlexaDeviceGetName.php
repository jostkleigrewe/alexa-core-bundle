...
//        $alexaUserId = $this->getAlexaRequest()
//            ->getSession()
//                ->getUser()
//                    ->getUserId();

        $alexaDeviceId = $this->alexaRequest
            ->getContext()
            ->getSystem()
            ->getDevice()
            ->getDeviceId();
...
//            ->findOneByAlexaId($alexaUserId);
        ...
        $alexaDevice = $this->getManager()
            ->getAlexaDeviceService()
            ->getAlexaDeviceRepository()
            ->findOneByDeviceId($alexaDeviceId);
        ...
        if ($alexaDevice) {
            if ($alexaDevice->getName()) {
                $this->alexaResponse
                    ->getResponse()
                    ->getOutputSpeech()
                    ->setText('Gerät bekannt und heißt ' . $alexaDevice->getName() . '.');
            } else {
                $this->alexaResponse
                    ->getResponse()
                    ->getOutputSpeech()
            ...
            $this->alexaResponse
                ->getResponse()
                ->getOutputSpeech()
                ->setText('Das aktuelle Gerät, von dem du sprichst, ist mir leider unbekannt');
