...
readonly class AlexaResponseService
{
    ...
    public function createResponse(AlexaRequest $alexaRequest): AlexaResponse
    {
        ...
        $sessionAttributes = $alexaRequest->getSession()->getAttributes() ?? [];
    ...
    public function createErrorResponse(Throwable $exception): AlexaResponse
    {
        ...
        // Setze den Status-Code auf 200, da Alexa immer 200 erwartet
        ...
        $response->getResponse()->getOutputSpeech()->setText($errorMessage);
        $response->getResponse()->getCard()->setTitle($errorTitle)
        ...
        if ($this->debugMode) {
            ...
            $response->getDebug()->setMessages($debugData);
        }

        // Kennzeichne, dass die Session nach der Antwort beendet wird
        $response->getResponse()->setShouldEndSession(true);
    ...
    {
        return sprintf(
            'Error of type %s in file %s, line %d: %s',
